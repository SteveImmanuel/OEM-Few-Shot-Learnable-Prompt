...
class OEMDataset(torch.utils.data.Dataset):
    ...
    def _generate_mask(self, img_shape: Tuple[int, int]):
        ...
        if np.random.rand() < self.half_mask_ratio or not self.is_train:
    ...
    def __getitem__(self, idx):
        ...
        img1, ori_label1 = self._load_img_lbl(*self.paths[pair_idx1])
        img2, ori_label2 = self._load_img_lbl(*self.paths[pair_idx2])
        if self.is_train and np.random.rand() > 0.5: # swap pair
            img1, img2 = img2, img1
            ori_label1, ori_label2 = ori_label2, ori_label1
