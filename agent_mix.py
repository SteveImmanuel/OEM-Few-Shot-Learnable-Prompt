...
class AgentMix():
    ...
    def visualize(self, title:str, imgs: T.Tensor, labels: T.Tensor, preds: T.Tensor, discretized_cmaps: T.Tensor, masks: T.Tensor, counter:int = None):
        if self.log_enabled and self.gpu_id == 0 and (self.counter % self.args['image_log_interval'] == 0 or counter is not None):
            # print("before", imgs.shape, labels.shape, preds.shape, discretized_cmaps.shape, masks.shape)
            # before torch.Size([1, 5, 3, 896, 448]) torch.Size([1, 5, 3, 896, 448]) torch.Size([1, 896, 448, 3]) torch.Size([1, 896, 448, 3]) torch.Size([1, 3, 896, 448])
            imgs = imgs[0][0][None, ...]
            labels = labels[0][0][None, ...]
            # print("after", imgs.shape, labels.shape, preds.shape, discretized_cmaps.shape, masks.shape)
    ...
    def iou(self, pred: T.tensor, label: T.tensor, mask: T.tensor, ori_label: T.tensor, color_palette: T.tensor):
        # print("before", pred.shape, label.shape, mask.shape, ori_label.shape)
        # torch.Size([5, 1568, 768]) torch.Size([1, 5, 3, 896, 448]) torch.Size([1, 5, 1568]) torch.Size([1, 5, 896, 448])
        pred = pred[0][None, ...]
        label = label[0][0][None, ...]
        mask = mask[0][0][None, ...]
        ori_label = ori_label[0][0][None, ...]
        # print("after", pred.shape, label.shape, mask.shape, ori_label.shape)
        # torch.Size([1, 1568, 768]) torch.Size([1, 3, 896, 448]) torch.Size([1, 1568]) torch.Size([1, 896, 448])
